...
# status and, if enabled, the interval
{
    ...
    if [ "$rest_value" = "0" ]; then
        ...
        if [ "$interval" != "false" ]; then
            echo "          enabled: *** disabled:     interval: $interval "
        else
...
# batch enable all inputs
{
    ...
    echo ""
    case $selection in
        1 ) enable_all_inputs; press_enter; manage_inputs_menu;;
...
# provides usage
{
    ...
    echo '       --enable-all    enable all inputs'
    echo '       --enable-input  input to be enabled'
       ...
       if [ ! -n "$action" ]; then
           ...
           if [ "$action" == "clone" ]; then 
               ...
               if [ "$token" == "all" ]; then
                   ...
                   case $token in
                       *.sh ) input_endpoint=$(build_scripted_input_endpoint "$token"); echo "enabling input $token"; echo ""; enable_scripted_input "$input_endpoint";;
                       * ) input_endpoint=$(build_monitor_input_endpoint "$token"); echo "enabling input $token"; echo ""; enable_monitor_input "$input_endpoint";;
                   esac
               fi
...
if [ ! -n "$1" ]; then 
    ...
    while [ "$1" != "" ]; do
        case $1 in
            ...
            --enable-all ) ACTION="enable"; ACTION_TARGET="all"; queue_action; shift;; 
            --enable-input ) ACTION="enable"; shift; ACTION_TARGET="$1"; queue_action; shift;; 
